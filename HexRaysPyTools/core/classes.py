...
class VirtualMethod(object):
    ...
    def setData(self, column, value):
        if column == 0:
            ...
            if len(split) == 2:
                ...
                if idaapi.parse_decl(tinfo, ida_compat.get_idati(), value, idaapi.PT_TYP) is not None:
                    if tinfo.is_func():
    ...
    def set_first_argument_type(self, name):
        ...
        if func_tinfo.get_func_details(func_data) and func_tinfo.get_nargs() and \
                class_tinfo.get_named_type(ida_compat.get_idati(), name):
...
class VirtualTable(object):
    ...
    def update(self):
        if self.modified:
            ...
            vtable_tinfo.get_numbered_type(ida_compat.get_idati(), self.ordinal)
    ...
    def update_local_type(self):
        if self.modified:
            ...
            if len(udt_data) == len(self.virtual_functions):
                ...
                final_tinfo.set_numbered_type(ida_compat.get_idati(), self.ordinal, idaapi.NTF_REPLACE, self.name)
                self.modified = False
            else:
                print("[ERROR] Something have been modified in Local types. Please refresh this view")
    ...
    def create(tinfo, class_):
        ordinal = idaapi.get_type_ordinal(ida_compat.get_idati(), tinfo.dstr())
        if ordinal == 0:
            if idaapi.import_type(ida_compat.get_idati(), -1, tinfo.dstr(), 0) == idaapi.BADNODE:
...
class Class(object):
    ...
    def update_local_type(self):
        if self.modified:
            ...
            tinfo.set_numbered_type(ida_compat.get_idati(), self.ordinal, idaapi.NTF_REPLACE, self.name)
            self.modified = False
    ...
    def tinfo(self):
        tinfo = idaapi.tinfo_t()
        tinfo.get_numbered_type(ida_compat.get_idati(), self.ordinal)
        return tinfo
