...
def choose_til():
    ...
    idati = ida_compat.get_idati()
    list_type_library = [(idati, idati.name, idati.desc)]
    for idx in range(ida_compat.get_idati().nbases):
        type_library = ida_compat.get_idati().base(idx)          # type: idaapi.til_t
...
def import_type(library, name):
    if library.name != ida_compat.get_idati().name:
        last_ordinal = idaapi.get_ordinal_qty(ida_compat.get_idati())
